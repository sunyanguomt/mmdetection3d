...
def test_anchor3d_head_loss():
    ...
    # test forward
    feats = list()
    feats.append(torch.rand([2, 512, 200, 176], dtype=torch.float32).musa())
    ...
    gt_bboxes = list(torch.empty((2, 0, 7)).musa())
    gt_labels = list(torch.empty((2, 0)).musa())
...
def test_anchor3d_head_getboxes():
    ...
    # test get_boxes
    cls_score[0] -= 1.5  # too many positive samples may cause musa oom
...
def test_vote_head():
    ...
    self = build_head(vote_head_cfg).musa()
    fp_xyz = [torch.rand([2, 256, 3], dtype=torch.float32).musa()]
    fp_features = [torch.rand([2, 256, 256], dtype=torch.float32).musa()]
    fp_indices = [torch.randint(0, 128, [2, 256]).musa()]
    ...
    # test loss
    points = [torch.rand([40000, 4], device='musa') for i in range(2)]
    gt_bbox1 = LiDARInstance3DBoxes(torch.rand([10, 7], device='musa'))
    gt_bbox2 = LiDARInstance3DBoxes(torch.rand([10, 7], device='musa'))
    gt_bboxes = [gt_bbox1, gt_bbox2]
    gt_labels = [torch.randint(0, 18, [10], device='musa') for i in range(2)]
    pts_semantic_mask = [
        torch.randint(0, 18, [40000], device='musa') for i in range(2)
    ]
    pts_instance_mask = [
        torch.randint(0, 10, [40000], device='musa') for i in range(2)
    ]
    ...
    obj_scores = torch.rand([256], device='musa')
    sem_scores = torch.rand([256, 18], device='musa')
    points = torch.rand([40000, 3], device='musa')
    bbox = torch.rand([256, 7], device='musa')
    ...
    # test get_boxes
    points = torch.rand([1, 40000, 4], device='musa')
    seed_points = torch.rand([1, 1024, 3], device='musa')
    seed_indices = torch.randint(0, 40000, [1, 1024], device='musa')
    vote_points = torch.rand([1, 1024, 3], device='musa')
    vote_features = torch.rand([1, 256, 1024], device='musa')
    aggregated_points = torch.rand([1, 256, 3], device='musa')
    aggregated_indices = torch.range(0, 256, device='musa')
    obj_scores = torch.rand([1, 256, 2], device='musa')
    center = torch.rand([1, 256, 3], device='musa')
    dir_class = torch.rand([1, 256, 1], device='musa')
    dir_res_norm = torch.rand([1, 256, 1], device='musa')
    dir_res = torch.rand([1, 256, 1], device='musa')
    size_class = torch.rand([1, 256, 18], device='musa')
    size_res = torch.rand([1, 256, 18, 3], device='musa')
    sem_scores = torch.rand([1, 256, 18], device='musa')
    bbox_preds = dict(
...
def test_part_aggregation_ROI_head():
    ...
    roi_head_cfg = _get_roi_head_cfg(
        'parta2/hv_PartA2_secfpn_2x8_cyclic_80e_kitti-3d-3class.py')
    self = build_head(roi_head_cfg).musa()

    features = np.load('./tests/test_samples/parta2_roihead_inputs.npz')
    seg_features = torch.tensor(
        features['seg_features'], dtype=torch.float32, device='musa')
    feats_dict = dict(seg_features=seg_features)

    voxels = torch.tensor(
        features['voxels'], dtype=torch.float32, device='musa')
    num_points = torch.ones([500], device='musa')
    coors = torch.zeros([500, 4], device='musa')
    voxel_centers = torch.zeros([500, 3], device='musa')
    box_type_3d = LiDARInstance3DBoxes
    ...
    gt_bboxes_3d = [LiDARInstance3DBoxes(torch.rand([5, 7], device='musa'))]
    gt_labels_3d = [torch.randint(0, 3, [5], device='musa')]
...
def test_free_anchor_3D_head():
    ...
    cls_scores = [
        torch.rand([4, 80, 200, 200], device='musa') for i in range(3)
    ]
    bbox_preds = [
        torch.rand([4, 72, 200, 200], device='musa') for i in range(3)
    ]
    dir_cls_preds = [
        torch.rand([4, 16, 200, 200], device='musa') for i in range(3)
    ]
    gt_bboxes = [
        LiDARInstance3DBoxes(torch.rand([8, 9], device='musa'), box_dim=9)
        for i in range(4)
    ]
    gt_labels = [
        torch.randint(0, 10, [8], device='musa', dtype=torch.long)
        for i in range(4)
    ]
    input_metas = [0]
...
def test_primitive_head():
    ...
    self = build_head(primitive_head_cfg).musa()
    fp_xyz = [torch.rand([2, 64, 3], dtype=torch.float32).musa()]
    hd_features = torch.rand([2, 256, 64], dtype=torch.float32).musa()
    fp_indices = [torch.randint(0, 64, [2, 64]).musa()]
    input_dict = dict(
    ...
    # test loss
    points = torch.rand([2, 1024, 3], dtype=torch.float32).musa()
    ret_dict['seed_points'] = fp_xyz[0]
    ret_dict['seed_indices'] = fp_indices[0]

    from mmdet3d.core.bbox import DepthInstance3DBoxes
    gt_bboxes_3d = [
        DepthInstance3DBoxes(torch.rand([4, 7], dtype=torch.float32).musa()),
        DepthInstance3DBoxes(torch.rand([4, 7], dtype=torch.float32).musa())
    ]
    gt_labels_3d = torch.randint(0, 18, [2, 4]).musa()
    gt_labels_3d = [gt_labels_3d[0], gt_labels_3d[1]]
    pts_semantic_mask = torch.randint(0, 19, [2, 1024]).musa()
    pts_semantic_mask = [pts_semantic_mask[0], pts_semantic_mask[1]]
    pts_instance_mask = torch.randint(0, 4, [2, 1024]).musa()
...
def test_h3d_head():
    ...
    self = build_head(h3d_head_cfg).musa()

    # prepare roi outputs
    fp_xyz = [torch.rand([1, num_point, 3], dtype=torch.float32).musa()]
    hd_features = torch.rand([1, 256, num_point], dtype=torch.float32).musa()
    fp_indices = [torch.randint(0, 128, [1, num_point]).musa()]
    aggregated_points = torch.rand([1, num_proposal, 3],
                                   dtype=torch.float32).musa()
    aggregated_features = torch.rand([1, 128, num_proposal],
                                     dtype=torch.float32).musa()
    proposal_list = torch.cat([
        torch.rand([1, num_proposal, 3], dtype=torch.float32).musa() * 4 - 2,
        torch.rand([1, num_proposal, 3], dtype=torch.float32).musa() * 4,
        torch.zeros([1, num_proposal, 1]).musa()
    ],
                              dim=-1)
    ...
    gt_labels_3d = torch.randint(0, 18, [1, 4]).musa()
    gt_labels_3d = [gt_labels_3d[0]]
    pts_semantic_mask = torch.randint(0, 19, [1, num_point]).musa()
    pts_semantic_mask = [pts_semantic_mask[0]]
    pts_instance_mask = torch.randint(0, 4, [1, num_point]).musa()
    pts_instance_mask = [pts_instance_mask[0]]
    points = torch.rand([1, num_point, 3], dtype=torch.float32).musa()

    # prepare rpn targets
    vote_targets = torch.rand([1, num_point, 9], dtype=torch.float32).musa()
    vote_target_masks = torch.rand([1, num_point], dtype=torch.float32).musa()
    size_class_targets = torch.rand([1, num_proposal],
                                    dtype=torch.float32).musa().long()
    size_res_targets = torch.rand([1, num_proposal, 3],
                                  dtype=torch.float32).musa()
    dir_class_targets = torch.rand([1, num_proposal],
                                   dtype=torch.float32).musa().long()
    dir_res_targets = torch.rand([1, num_proposal], dtype=torch.float32).musa()
    center_targets = torch.rand([1, 4, 3], dtype=torch.float32).musa()
    mask_targets = torch.rand([1, num_proposal],
                              dtype=torch.float32).musa().long()
    valid_gt_masks = torch.rand([1, 4], dtype=torch.float32).musa()
    ...
    box_loss_weights = torch.rand([1, num_proposal],
                                  dtype=torch.float32).musa()
    valid_gt_weights = torch.rand([1, 4], dtype=torch.float32).musa()
...
def test_dcn_center_head():
    ...
    dcn_center_head = build_head(dcn_center_head_cfg).musa()

    x = torch.ones([2, 384, 128, 128]).musa()
    ...
    # Test loss.
    gt_bboxes_0 = LiDARInstance3DBoxes(torch.rand([10, 9]).musa(), box_dim=9)
    gt_bboxes_1 = LiDARInstance3DBoxes(torch.rand([20, 9]).musa(), box_dim=9)
    gt_labels_0 = torch.randint(1, 11, [10]).musa()
    gt_labels_1 = torch.randint(1, 11, [20]).musa()
...
def test_ssd3d_head():
    ...
    self = build_head(ssd3d_head_cfg).musa()
    sa_xyz = [torch.rand([2, 128, 3], dtype=torch.float32).musa()]
    sa_features = [torch.rand([2, 256, 128], dtype=torch.float32).musa()]
    sa_indices = [torch.randint(0, 64, [2, 128]).musa()]
    ...
    # test loss
    points = [torch.rand([4000, 3], device='musa') for i in range(2)]
    gt_bbox1 = LiDARInstance3DBoxes(torch.rand([5, 7], device='musa'))
    gt_bbox2 = LiDARInstance3DBoxes(torch.rand([5, 7], device='musa'))
    gt_bboxes = [gt_bbox1, gt_bbox2]
    gt_labels = [
        torch.zeros([5], dtype=torch.long, device='musa') for i in range(2)
    ]
...
def test_shape_aware_head_loss():
    ...
    # test forward
    feats = list()
    feats.append(torch.rand([2, 384, 200, 200], dtype=torch.float32).musa())
    (cls_score, bbox_pred, dir_cls_preds) = self.forward(feats)
    ...
    gt_labels = list(torch.tensor([[4, 4], [4, 4]], dtype=torch.int64).musa())
    input_metas = [{
        'sample_idx': 1234
    }, {
        'sample_idx': 2345
...
def test_fcos_mono3d_head():
    ...
    self = build_head(fcos3d_head_cfg).musa()

    feats = [
        torch.rand([2, 256, 116, 200], dtype=torch.float32).musa(),
        torch.rand([2, 256, 58, 100], dtype=torch.float32).musa(),
        torch.rand([2, 256, 29, 50], dtype=torch.float32).musa(),
        torch.rand([2, 256, 15, 25], dtype=torch.float32).musa(),
        torch.rand([2, 256, 8, 13], dtype=torch.float32).musa()
    ]
    ...
    # test loss
    gt_bboxes = [
        torch.rand([3, 4], dtype=torch.float32).musa(),
        torch.rand([3, 4], dtype=torch.float32).musa()
    ]
    gt_bboxes_3d = CameraInstance3DBoxes(
        torch.rand([3, 9], device='musa'), box_dim=9)
    gt_labels = [torch.randint(0, 10, [3], device='musa') for i in range(2)]
    gt_labels_3d = gt_labels
    centers2d = [
        torch.rand([3, 2], dtype=torch.float32).musa(),
        torch.rand([3, 2], dtype=torch.float32).musa()
    ]
    depths = [
        torch.rand([3], dtype=torch.float32).musa(),
        torch.rand([3], dtype=torch.float32).musa()
    ]
    attr_labels = [torch.randint(0, 9, [3], device='musa') for i in range(2)]
    img_metas = [
        dict(
...
def test_groupfree3d_head():
    ...
    fp_xyz = [torch.rand([2, 256, 3], dtype=torch.float32).musa()]
    fp_features = [torch.rand([2, 288, 256], dtype=torch.float32).musa()]
    fp_indices = [torch.randint(0, 128, [2, 256]).musa()]
    ...
    # test losses
    points = [torch.rand([50000, 4], device='musa') for i in range(2)]
    gt_bbox1 = torch.rand([10, 7], dtype=torch.float32).musa()
    gt_bbox2 = torch.rand([10, 7], dtype=torch.float32).musa()
    ...
    labels_1 = torch.randint(0, 18, [10], device='musa')
    labels_2 = torch.randint(0, 18, [10], device='musa')
    ...
    obj_scores = torch.rand([256], device='musa')
    sem_scores = torch.rand([256, 18], device='musa')
    points = torch.rand([50000, 3], device='musa')
    bbox = torch.rand([256, 7], device='musa')
    input_meta = dict(box_type_3d=DepthInstance3DBoxes)
    ...
    # test get_boxes
    points = torch.rand([1, 50000, 3], device='musa')
    seed_points = torch.rand([1, 1024, 3], device='musa')
    seed_indices = torch.randint(0, 50000, [1, 1024], device='musa')
    obj_scores = torch.rand([1, 256, 1], device='musa')
    center = torch.rand([1, 256, 3], device='musa')
    dir_class = torch.rand([1, 256, 1], device='musa')
    dir_res_norm = torch.rand([1, 256, 1], device='musa')
    dir_res = torch.rand([1, 256, 1], device='musa')
