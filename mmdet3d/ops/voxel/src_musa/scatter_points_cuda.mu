...
std::vector<at::Tensor> dynamic_point_to_voxel_forward_gpu(
  ...
  AT_DISPATCH_FLOATING_TYPES(
    ...
    dim3 blocks(std::min(at::musa::ATenCeilDiv(num_input, threadsPerBlock),
                         maxGridDim));
...
void dynamic_point_to_voxel_backward_gpu(at::Tensor &grad_feats,
  ...
  if (reduce_type == reduce_t::MEAN || reduce_type == reduce_t::SUM) {
    AT_DISPATCH_FLOATING_TYPES(
        ...
        ([&] {
          dim3 blocks(std::min(
              at::musa::ATenCeilDiv(num_input, threadsPerBlock), maxGridDim));
          dim3 threads(threadsPerBlock);
    ...
    AT_DISPATCH_FLOATING_TYPES(
        ...
        "max_reduce_traceback_scatter_idx_kernel", ([&] {
          dim3 blocks(std::min(
              at::musa::ATenCeilDiv(num_reduced, threadsPerBlock), maxGridDim));
          dim3 threads(threadsPerBlock);
