...
#define CHECK_CUDA(x) \
  TORCH_CHECK(x.device().is_privateuseone(), #x, " must be a CUDAtensor ")
...
inline void gpuAssert(musaError_t code, const char *file, int line,
                      bool abort = true) {
  if (code != musaSuccess) {
    fprintf(stderr, "GPUassert: %s %s %d\n", musaGetErrorString(code), file,
            line);
    if (abort) exit(code);
  }
}
...
int nms_gpu(at::Tensor boxes, at::Tensor keep,
  ...
  CHECK_ERROR(musaMalloc((void **)&mask_data,
  ...
  CHECK_ERROR(musaMemcpy(&mask_cpu[0], mask_data,
                         boxes_num * col_blocks * sizeof(unsigned long long),
  ...
  delete[] remv_cpu;
  if (musaSuccess != musaGetLastError()) printf("Error!\n");
